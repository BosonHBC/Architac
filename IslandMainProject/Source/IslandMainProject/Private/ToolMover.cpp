...
void AToolMover::UseTool()
{
	if (!m_Using && m_currentTool) {
		...
		// Reduce speed to 0%
		m_venturePawn->ChangeCharacterVelocity(100);
		// Block 100% rotation;
		m_venturePawn->BlockCharacterRotation(100);
...
void AToolMover::UseToolFinish()
{
	if (m_Using && m_currentTool && m_ToolReady) {
		m_Using = false;
		m_currentTool->UseToolFinish();
		m_venturePawn->ChangeCharacterVelocity(600);
		// Reset block
		m_venturePawn->BlockCharacterRotation(640);
		BPUseToolFinish();
	}
}

void AToolMover::SetVenturePawnReference(class AVenturePawn* venturePawnRef)
{
	m_venturePawn = venturePawnRef;
}
